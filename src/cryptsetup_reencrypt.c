...
static int set_reencrypt_requirement(const struct reenc_ctx *rc)
{
	...
	if (reqs & CRYPT_REQUIREMENT_REENCRYPT_LEGACY) {
	...
	r = crypt_persistent_flags_set(cd, CRYPT_FLAGS_REQUIREMENTS, reqs | CRYPT_REQUIREMENT_REENCRYPT_LEGACY);
out:
	crypt_free(cd);
	return r;
}
...
static int device_check(struct reenc_ctx *rc, header_magic set_magic)
{
	...
	if (set_magic == MAKE_UNUSABLE && !memcmp(buf, MAGIC, MAGIC_L) &&
		...
		log_verbose(_("Setting LUKS2 reencrypt legacy flag on device %s.\n"), rc->device);
...
int main(int argc, const char **argv)
{
	...
	static struct poptOption popt_options[] = {
		...
		{ "type",              '\0', POPT_ARG_STRING, &opt_type,                0, N_("The LUKS type."), NULL },
		POPT_TABLEEND
	};
