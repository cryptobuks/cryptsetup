...
{
	return reqs & CRYPT_REQUIREMENT_REENCRYPT_LEGACY;
}
		     ...
		     const char *backup_file)
{
	...
	r = snprintf(msg, sizeof(msg), _("Device %s %s%s%s%s"), device_path(device),
		     ...
		     reqs_reencrypt(reqs) ? _("\nWARNING: real LUKS2 device header marked for legacy reencryption!"
					      "\nReplacing header with backup may corrupt the data on that device!") : "");
...
static const struct  {
	...
	{ CRYPT_REQUIREMENT_REENCRYPT_LEGACY, "reencrypt" },
	{ 0, NULL }
};
...
int LUKS2_unmet_requirements(struct crypt_device *cd, struct luks2_hdr *hdr, uint32_t reqs_mask, int quiet)
{
	...
	if (reqs_reencrypt(reqs) && !quiet)
		log_err(cd, _("Legacy reencryption in progress. Aborting.\n"));
